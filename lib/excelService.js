...
class ExcelService {
  static buildWorksheet(applications) {
    return XLSX.utils.json_to_sheet(applications.map(app => ({
      'Email': app.email,
      'Name': app.name || '',
      'Company': app.company || '',
      'HR Number': app.hrNumber || '',
  ...
  static exportToExcel(applications, filename = 'job_applications.xlsx') {
    const worksheet = ExcelService.buildWorksheet(applications);
  ...
  static exportToBuffer(applications) {
    const worksheet = ExcelService.buildWorksheet(applications);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Job Applications');
  ...
  static importFromExcel(filePath) {
    try {
      ...
      return data.map(row => ({
        ...
        hrNumber: row['HR Number'] || row.hrNumber || '',
